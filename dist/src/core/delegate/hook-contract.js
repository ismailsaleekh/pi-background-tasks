 ...
 */
export const DELEGATE_HOOK_CONTRACT_SCHEMA_VERSION = 'pi-background-tasks.delegate-hook-contract.v1';
/**
 ...
 */
export const DELEGATE_HOOK_CONTRACT_ID = 'context-measure-abort-v1+tool-result-spill-v1';
 ...
 */
export const DELEGATE_REQUIRED_HOOK_GUARANTEES = [
    'context_fires_before_every_model_call',
    'context_result_messages_reach_provider',
    'context_abort_blocks_provider_call',
    'context_abort_terminates_run',
...
export function evaluateDelegateHookContract(evidence) {
    const missing = DELEGATE_REQUIRED_HOOK_GUARANTEES.filter((guarantee) => evidence.guarantees[guarantee] !== true);
...
export function parseDelegateHookContractEvidence(value) {
    if (!isRecord(value))
        throw new Error('delegate hook-contract evidence must be an object');
    if (value['schema_version'] !== DELEGATE_HOOK_CONTRACT_SCHEMA_VERSION) {
        throw new Error(`delegate hook-contract evidence schema_version must be ${DELEGATE_HOOK_CONTRACT_SCHEMA_VERSION}`);
    }
    if (value['contract_id'] !== DELEGATE_HOOK_CONTRACT_ID) {
        throw new Error(`delegate hook-contract evidence contract_id must be ${DELEGATE_HOOK_CONTRACT_ID}`);
    }
    const raw = value['guarantees'];
    if (!isRecord(raw))
        throw new Error('delegate hook-contract evidence guarantees must be an object');
    const keys = Object.keys(raw).sort();
    const expected = [...DELEGATE_HOOK_GUARANTEE_NAMES].sort();
    if (keys.length !== expected.length || keys.some((key, index) => key !== expected[index])) {
        throw new Error(`delegate hook-contract evidence guarantees keys mismatch: expected ${expected.join(', ')}`);
    }
    const flag = (name) => {
