import { TOKEN_BUDGET_CALIBRATION_VERSION, TOKEN_BUDGET_FAMILY_CALIBRATIONS, TOKEN_BUDGET_RATE_SCALE, estimateInputTokens, knownTextSegment, resolveTokenBudgetFamily, utf8ByteClassBreakdown, allowedInputTokens, isUsableContextWindow, } from '../context/token-budget.js';
import { DELEGATE_DEFAULT_MAX_TOOL_CALLS, DELEGATE_DEFAULT_MAX_TURNS, DELEGATE_DEFAULT_TIMEOUT_SECONDS, DELEGATE_INLINE_ANSWER_BYTES, } from './facade-contract.js';
import { DELEGATE_BUDGET_PLAN_SCHEMA_VERSION, DelegateError, } from './types.js';
export { DELEGATE_DEFAULT_MAX_TOOL_CALLS, DELEGATE_DEFAULT_MAX_TURNS, DELEGATE_DEFAULT_TIMEOUT_SECONDS, DELEGATE_INLINE_ANSWER_BYTES, } from './facade-contract.js';
/**
 * Delegate budgeting.
 *
...
export const DELEGATE_MIN_CONTEXT_WINDOW_TOKENS = DELEGATE_MIN_USABLE_INPUT_TOKENS +
...
export const DELEGATE_BUDGET_POLICY = {
 ...
 */
export function delegateAllowedInputTokens(route) {
    if (!isUsableContextWindow(route.context_window_tokens)) {
        throw new DelegateError(`bg_delegate route ${route.qualified_id} reports no usable context-window capacity`, {
...
export function planDelegateAdmission(input) {
    ...
    const provableConservativeLaunchTokens = conservativeEstimate.advisory.input_tokens_if_multibyte_used_provable_ceiling;
    const byteCapacity = Math.floor((allowed * estimate.rateSource.effective_rate_bytes_per_token_x100) / TOKEN_BUDGET_RATE_SCALE);
    const launchBytes = childPrompt.bytes + system.bytes;
    const retainedGrowthBudget = Math.max(0, allowed - estimate.tokens - DELEGATE_FINALIZATION_INPUT_RESERVE_TOKENS);
    return {
...
function rateWarningText(rateSource, qualifiedId) {
    if (rateSource.warning === null)
        return '';
    return ` Estimator warning for ${qualifiedId}: ${rateSource.warning}.`;
}
function requiredByteReduction(plan) {
    const variableTokens = plan.route.allowed_input_tokens - plan.route.rate_source.affine_f_tokens;
    const maximumBytes = variableTokens <= 0
        ? 0
        : Math.floor((variableTokens * plan.route.rate_source.effective_rate_bytes_per_token_x100) /
 ...
 */
export function evaluateDelegateRuntimeBudget(measurement, allowedTokens, route) {
...
export function resolveDelegateLimits(route, overrides = {}) {
    return {
        max_turns: positiveInteger(overrides.maxTurns, DELEGATE_DEFAULT_MAX_TURNS, 'maxTurns'),
        max_tool_calls: positiveInteger(overrides.maxToolCalls, DELEGATE_DEFAULT_MAX_TOOL_CALLS, 'maxToolCalls'),
        timeout_seconds: positiveInteger(overrides.timeoutSeconds, DELEGATE_DEFAULT_TIMEOUT_SECONDS, 'timeoutSeconds'),
