...
import { DELEGATE_RECEIPT_SCHEMA_VERSION, DELEGATE_RESULT_PACKAGE_SCHEMA_VERSION, DelegateError, } from './types.js';
/**
...
function fail(message, code, taskId) {
    const details = { code, childCreated: true };
    if (taskId !== undefined)
 ...
 */
export function assertWellFormedUtf8(text, label) {
    const encoded = Buffer.from(text, 'utf8');
    if (encoded.toString('utf8') !== text) {
        fail(`${label} contains text that cannot be represented in UTF-8 without substitution (lone surrogate); the answer is not silently repaired`, 'child_result_encoding_invalid');
    }
    return encoded;
}
export function buildDelegateResultPackage(input) {
    const blocks = input.answerBlocks.map((text, index) => {
...
export function serializeDelegateResultPackage(pkg) {
...
function requireInteger(record, key, taskId) {
    const value = record[key];
    if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0)
        fail(`delegate result package field ${key} must be a non-negative safe integer`, 'child_result_invalid', taskId);
    return value;
}
const SHA256_PATTERN = /^[0-9a-f]{64}$/;
function requireSha256(record, key, taskId) {
    const value = requireString(record, key, taskId);
    if (!SHA256_PATTERN.test(value))
        fail(`delegate result package field ${key} must be a lowercase SHA-256 digest`, 'child_result_invalid', taskId);
    return value;
}
function parseUsage(value, taskId) {
    if (!isRecord(value))
    ...
    if (status === 'unavailable') {
        ...
        if (typeof reason !== 'string')
            fail('delegate result package unavailable usage must state a reason', 'child_result_invalid', taskId);
        return { status: 'unavailable', reason };
    }
    if (status !== 'observed')
        fail('delegate result package usage status must be observed or unavailable', 'child_result_invalid', taskId);
    const usage = value['usage'];
    if (!isRecord(usage))
        fail('delegate result package observed usage must be an object', 'child_result_invalid', taskId);
    const cost = usage['cost'];
    if (!isRecord(cost))
        fail('delegate result package observed usage must carry a complete cost breakdown', 'child_result_invalid', taskId);
    const number = (record, key) => {
        const raw = record[key];
        if (typeof raw !== 'number' || !Number.isFinite(raw) || raw < 0)
            fail(`delegate result package usage.${key} must be a non-negative finite number`, 'child_result_invalid', taskId);
        return raw;
    };
    return {
        status: 'observed',
...
function parseAttestations(value, taskId) {
    if (!Array.isArray(value))
        fail('delegate result package route_attestations must be an array', 'route_attestation_missing', taskId);
    if (value.length === 0)
        fail('delegate result package carries no route attestation, so the route the child actually used cannot be proven', 'route_attestation_missing', taskId);
...
function parseSpillContentFormat(value, taskId) {
    if (value === undefined)
        return undefined;
    if (value !== 'single_text_utf8' &&
...
function parseSpillReceipts(value, taskId) {
    if (!Array.isArray(value))
 ...
 */
export function verifyDelegateResultPackage(raw, expected) {
    let parsed;
    try {
        parsed = JSON.parse(raw);
    }
    catch (error) {
        fail(`delegate result package is not valid JSON: ${error instanceof Error ? error.message : String(error)}`, 'child_result_invalid', expected.taskId);
    }
    if (!isRecord(parsed))
    ...
    if (parsed['schema_version'] !== DELEGATE_RESULT_PACKAGE_SCHEMA_VERSION)
        fail(`delegate result package schema_version must be ${DELEGATE_RESULT_PACKAGE_SCHEMA_VERSION}`, 'child_result_invalid', expected.taskId);
    ...
    if (taskId !== expected.taskId || launchNonce !== expected.launchNonce)
        fail('delegate result package identity does not match the launched task; a stale or foreign package is never accepted', 'child_result_invalid', expected.taskId);
    const seedSha256 = requireSha256(parsed, 'seed_sha256', expected.taskId);
    if (seedSha256 !== expected.seedSha256)
        fail(`delegate result package was produced from a different seed: expected ${expected.seedSha256}, package declares ${seedSha256}`, 'seed_hash_mismatch', expected.taskId);
    ...
    if (provider !== expected.route.provider || model !== expected.route.model)
        fail(`delegate result package route ${provider}/${model} does not match the pinned route ${expected.route.provider}/${expected.route.model}`, 'route_mismatch', expected.taskId);
    const attestations = parseAttestations(parsed['route_attestations'], expected.taskId);
    for (const attestation of attestations) {
        if (attestation.provider !== expected.route.provider ||
            attestation.model !== expected.route.model)
            fail(`delegate child produced an assistant message on ${attestation.provider}/${attestation.model}, but the pinned route is ${expected.route.provider}/${expected.route.model}`, 'route_mismatch', expected.taskId);
    }
    ...
    if (answerRecord['encoding'] !== 'utf-8')
        fail('delegate result package answer encoding must be utf-8', 'child_result_encoding_invalid', expected.taskId);
    ...
    for (const [index, entry] of blocksValue.entries()) {
        if (!isRecord(entry))
            fail(`delegate answer block ${String(index)} must be an object`, 'child_result_invalid', expected.taskId);
        if (entry['kind'] !== 'text')
            fail(`delegate answer block ${String(index)} kind must be text`, 'child_result_invalid', expected.taskId);
        ...
        if (bytes.toString('base64') !== dataBase64)
            fail(`delegate answer block ${String(index)} is not strict base64`, 'child_result_invalid', expected.taskId);
        if (bytes.length !== byteLength)
            fail(`delegate answer block ${String(index)} declares ${String(byteLength)} bytes but carries ${String(bytes.length)}`, 'answer_hash_mismatch', expected.taskId);
        const actual = sha256Bytes(bytes);
        if (actual !== sha)
            fail(`delegate answer block ${String(index)} hash mismatch: declared ${sha}, computed ${actual}`, 'answer_hash_mismatch', expected.taskId);
    ...
    if (aggregate.length !== declaredLength)
        fail(`delegate answer declares ${String(declaredLength)} bytes but its blocks total ${String(aggregate.length)}`, 'answer_hash_mismatch', expected.taskId);
    const aggregateSha = sha256Bytes(aggregate);
    if (aggregateSha !== declaredSha)
        fail(`delegate answer aggregate hash mismatch: declared ${declaredSha}, computed ${aggregateSha}`, 'answer_hash_mismatch', expected.taskId);
    const answer = aggregate.toString('utf8');
    if (!Buffer.from(answer, 'utf8').equals(aggregate))
        fail('delegate answer bytes are not well-formed UTF-8; they are preserved on disk and never returned with substitution characters', 'child_result_encoding_invalid', expected.taskId);
    return {
        package: {
            schema_version: DELEGATE_RESULT_PACKAGE_SCHEMA_VERSION,
            task_id: taskId,
