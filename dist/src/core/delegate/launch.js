...
import { assertDelegateAdmission, planDelegateAdmission, resolveDelegateLimits, } from './budget.js';
import { buildDelegateSeed } from './seed.js';
import { DELEGATE_CAPABILITIES, DELEGATE_TOOL_NAME, DelegateError, } from './types.js';
import { DELEGATE_REQUIRED_HOOK_GUARANTEES, evaluateDelegateHookContract, parseDelegateHookContractEvidence, } from './hook-contract.js';
/**
...
export function makeDelegateTaskId() {
    return `d${randomBytes(16).toString('hex')}`;
}
export function makeDelegateLaunchNonce() {
    return randomBytes(16).toString('hex');
}
/** Child session ids are random and never derived from the parent session. */
export function makeDelegateChildSessionId() {
 ...
 * convenience.
 */
export function resolveDelegateRoute(input) {
    if (input.requested !== undefined) {
        const { provider, model } = input.requested;
        const found = input.availableModels.find((candidate) => candidate.provider === provider && candidate.id === model);
        if (found === undefined) {
            throw new DelegateError(`bg_delegate route ${provider}/${model} is not available in this session's model registry. No substitute route was selected and no child was created.`, {
    ...
    if (current === undefined) {
        throw new DelegateError('bg_delegate cannot pin a route: the parent session has no current model and no explicit route was given. No child was created.', {
            code: 'route_unresolved',
            childCreated: false,
...
function pinned(candidate, thinkingLevel, origin) {
    const contextWindow = candidate.contextWindow;
    if (contextWindow === undefined) {
        throw new DelegateError(`bg_delegate route ${candidate.provider}/${candidate.id} declares no context window, so its capacity cannot be verified before launch. No capacity was assumed and no child was created.`, {
 ...
 */
export function assertDelegateHookContract(evidence) {
    const verdict = evaluateDelegateHookContract(evidence);
    if (verdict.supported)
        return;
    throw new DelegateError(`bg_delegate cannot run on this Pi build: the child-side guard requires hook guarantees that were not observed (${verdict.missing.join(', ')}). No child was created.`, {
...
export function loadDelegateHookContractEvidence(raw) {
    let parsed;
    try {
        parsed = JSON.parse(raw);
    }
    catch (error) {
        throw new DelegateError(`bg_delegate hook-contract evidence is unreadable: ${error instanceof Error ? error.message : String(error)}`, { code: 'delegate_hook_contract_unsupported', childCreated: false });
    }
    try {
        return parseDelegateHookContractEvidence(parsed);
    }
    catch (error) {
        throw new DelegateError(`bg_delegate hook-contract evidence is invalid: ${error instanceof Error ? error.message : String(error)}`, { code: 'delegate_hook_contract_unsupported', childCreated: false });
    }
}
export function resolveDelegateChildExtensionPath(moduleUrl = import.meta.url, pathExists = existsSync) {
 ...
 */
export const DELEGATE_INSPECT_TOOLS = [
    'read',
    'grep',
    'find',
    'ls',
...
export const DELEGATE_FORBIDDEN_TOOLS = [
    'bash',
    'edit',
    'write',
    'bg_run',
...
export function delegateToolsFor(capability) {
    if (capability === 'inspect')
        return DELEGATE_INSPECT_TOOLS;
...
function assertDelegateExtensionMode(mode) {
    if (mode === 'isolated' || mode === 'ambient')
        return;
 ...
 */
export function buildDelegateChildArgv(input) {
    ...
    for (const forbidden of DELEGATE_FORBIDDEN_TOOLS) {
        if (tools.includes(forbidden)) {
            throw new DelegateError(`bg_delegate capability ${input.capability} would enable the forbidden tool ${forbidden}`, { code: 'delegate_isolation_unsupported', childCreated: false });
        }
    }
    const extensionPaths = input.route.provider === 'anthropic'
        ? [
            input.attributionExtensionPath ??
                (() => {
                    throw new DelegateError('Anthropic delegate launch requires the package attribution extension', { code: 'delegate_isolation_unsupported', childCreated: false });
                })(),
...
export function delegateChildEnv(input, env = process.env) {
    const out = { ...env };
    for (const key of DELEGATE_REMOVED_ENV_KEYS)
 ...
 */
export function buildDelegateChildSystemPrompt(seedPathHint) {
    return [
 ...
 */
export function buildDelegateChildPrompt(seedSerialized, directive) {
    return [
 ...
 */
export function preflightDelegateLaunch(input) {
...
export async function ensureDelegateChildSessionDir(artifactDirAbs) {
