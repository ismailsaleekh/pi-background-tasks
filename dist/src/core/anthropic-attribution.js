...
function buildAnthropicRequest(model, context, options) {
    ...
    const systemPrompt = context.systemPrompt ?? transcript.getCurrentSystemPrompt?.(context.messages) ?? '';
    const system = systemPrompt.trim().length > 0
        ...
        : [];
    const tools = convertTools(context.tools ?? transcript.getCurrentTools?.(context.messages), model.compat?.supportsCacheControlOnTools === false ? undefined : cacheControl);
