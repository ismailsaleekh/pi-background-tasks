...
function ledgerLeafHash(index, record) {
...
function ledgerRootHash(leaves) {
...
function unsupportedBlock(label) {
...
function compactCounts(counts) {
    const out = {};
    if (counts.assistant_thinking > 0)
        out.assistant_thinking = counts.assistant_thinking;
    if (counts.tool_calls > 0)
        out.tool_calls = counts.tool_calls;
    if (counts.tool_result_texts > 0)
        out.tool_result_texts = counts.tool_result_texts;
    return out;
}
function imageMarker(mimeType) {
...
class ProjectionBuilder {
    ...
    pendingCounts;
    pendingBytes = 0;
    pendingLedgerFirst = 0;
    pendingLedgerLast = 0;
    pendingSourceFirst = 0;
    pendingSourceLast = 0;
    ...
    addText(entry) {
        ...
        if (entry.role === 'user')
            this.includedUserTextBytes += utf8Bytes(entry.text);
        else
            this.includedAssistantTextBytes += utf8Bytes(entry.text);
    }
    addOmission(sourceOrdinal, blockOrdinal, kind, payload, extra = {}) {
        ...
        if (extra.toolName !== undefined)
            record.tool_name = extra.toolName;
        if (extra.toolCallId !== undefined)
            record.tool_call_id = extra.toolCallId;
        if (extra.mimeType !== undefined)
            record.mime_type = extra.mimeType;
    ...
    finish(messageCount) {
        ...
        for (const row of this.ledger) {
            if (row.kind === 'assistant_thinking')
                thinkingBytes += row.payload_bytes;
        ...
        const toolCallNames = [...this.toolCallNames.entries()]
 ...
 */
function projectUserMessage(builder, content, sourceOrdinal) {
...
function projectAssistantMessage(builder, content, sourceOrdinal) {
    for (const [blockOrdinal, block] of content.entries()) {
        if (block.type === 'text') {
            if (block.text.length === 0) {
                builder.emptyTextBlocks += 1;
        ...
        if (block.type === 'thinking') {
            builder.addOmission(sourceOrdinal, blockOrdinal, 'assistant_thinking', Buffer.from(block.thinking, 'utf8'));
            continue;
        }
        if (block.type === 'toolCall') {
            builder.addOmission(sourceOrdinal, blockOrdinal, 'tool_call', Buffer.from(canonicalJson(block.arguments), 'utf8'), { toolName: block.name, toolCallId: block.id });
            continue;
        }
        throw unsupportedBlock(`assistant block ${String(Reflect.get(block, 'type'))}`);
    }
}
function projectToolResultMessage(builder, message, sourceOrdinal) {
    ...
    for (const [blockOrdinal, block] of content.entries()) {
        if (block.type === 'text') {
            builder.addOmission(sourceOrdinal, blockOrdinal, 'tool_result_text', Buffer.from(block.text, 'utf8'), { toolName: message.toolName, toolCallId: message.toolCallId });
            continue;
        }
        if (block.type === 'image') {
            builder.addOmission(sourceOrdinal, blockOrdinal, 'tool_result_image', Buffer.from(block.data, 'utf8'), {
                toolName: message.toolName,
                toolCallId: message.toolCallId,
 ...
 */
export function projectVisibleConversationV2(messages) {
    const builder = new ProjectionBuilder();
    for (const [sourceOrdinal, message] of messages.entries()) {
        if (message.role === 'user')
            projectUserMessage(builder, message.content, sourceOrdinal);
            ...
            throw unsupportedBlock(`message role ${String(Reflect.get(message, 'role'))}`);
    }
    return builder.finish(messages.length);
}
//# sourceMappingURL=visible-conversation-v2.js.map