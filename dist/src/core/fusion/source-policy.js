...
import { FUSION_SOURCE_POLICY_SCHEMA_VERSION, FusionError, } from './types.js';
...
export function canonicalizeFusionPublicUrl(value) {
    let url;
    try {
        url = new URL(value);
    }
    catch (error) {
        throw new FusionError(`fusion research declared source URL is malformed: ${error instanceof Error ? error.message : String(error)}`, { code: 'orchestration_failed', childCreated: false });
    }
...
export function normalizeFusionDeclaredSources(sources = []) {
    const seen = new Map();
    return sources.map((source, index) => {
        ...
        if (previous !== undefined) {
            throw new FusionError(`fusion research source ${String(index)} duplicates canonical URL from source ${String(previous)}: ${canonicalUrl}`, { code: 'orchestration_failed', childCreated: false });
        }
...
export function buildFusionSourcePolicy(cwd, sources) {
    const body = {
...
export function sourcePolicyCanonicalBytes(policy) {
...
export function parseFusionSourcePolicy(value) {
    ...
    const sources = value['sources'].map((item, index) => {
        ...
        if (itemKeys.join('\0') !== itemExpected.join('\0'))
        ...
        if (!SHA256_HEX.test(sha256))
            throw new Error(`${label}.sha256 must be sha256`);
        if (canonicalizeFusionPublicUrl(url) !== canonical_url)
            throw new Error(`${label}.canonical_url mismatch`);
        if (url !== canonical_url)
            throw new Error(`${label}.url must equal canonical_url`);
        if (purpose.trim() !== purpose)
            throw new Error(`${label}.purpose must be trimmed`);
...
async function readRegularFileNoSymlink(path, label) {
    let handle;
    try {
...
export async function readFusionSourcePolicyFile(path, expectedSha256) {
    ...
    if (actual !== expectedSha256)
        throw new Error('fusion source policy artifact hash mismatch');
