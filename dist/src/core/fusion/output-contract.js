...
const FUSION_CANDIDATE_MAX_OUTPUT_BYTES_DISPLAY = FUSION_CANDIDATE_MAX_OUTPUT_BYTES.toLocaleString('en-US');
...
export function fusionJsonRenderedTextBytes(text) {
    return Buffer.byteLength(JSON.stringify(text), 'utf8');
}
export function fusionOutputContractBytes(stage) {
    if (stage === 'candidate')
        return FUSION_CANDIDATE_MAX_OUTPUT_BYTES;
    if (stage === 'evaluation')
        return FUSION_EVALUATION_MAX_OUTPUT_BYTES;
    return FUSION_MERGE_MAX_OUTPUT_BYTES;
}
export function assertChildOutputWithinContract(stage, text) {
    const bytes = fusionJsonRenderedTextBytes(text);
    const allowed = fusionOutputContractBytes(stage);
    if (bytes <= allowed)
        return;
    throw new FusionError(`fusion ${stage} response is ${String(bytes)} JSON-rendered bytes, exceeding the ${String(allowed)}-byte output contract for that stage; the response is preserved in the run artifacts and is not forwarded or truncated`, { code: 'child_output_cap', stage, childCreated: true });
}
