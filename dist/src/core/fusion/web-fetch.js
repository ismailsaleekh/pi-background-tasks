...
export async function fusionWebFetch(req, options = {}) {
    ...
    for (let redirectCount = 0;; redirectCount += 1) {
        ...
        if (result.kind === 'redirect') {
            if (redirectCount >= effective.maxRedirects) {
                throw new FusionWebFetchError('redirect_limit', `fusion_web_fetch redirect limit exceeded (${String(effective.maxRedirects)})`, { url: currentUrl.toString(), status: result.status });
            }
            currentUrl = normalizeRedirectUrl(currentUrl, result.location);
            continue;
        }
        const extracted = await extractWithinDeadline(result.body, result.contentType, requestedFormat, effective, deadlineMs, currentUrl);
...
function fetchTimeout(url, phase) {
    return new FusionWebFetchError('request_timeout', `fusion_web_fetch request timeout elapsed during ${phase}`, { url: url.toString() });
}
function assertFetchDeadline(options, deadlineMs, url, phase) {
    const remainingMs = deadlineMs - options.now();
    if (remainingMs <= 0)
        throw fetchTimeout(url, phase);
    return remainingMs;
}
async function extractWithinDeadline(body, contentType, requestedFormat, options, deadlineMs, url) {
    ...
    try {
        ...
        const extraction = Promise.resolve().then(() => options.extractContent(body, contentType, requestedFormat));
...
function normalizeRequestUrl(rawUrl) {
    ...
    try {
        ...
        throw new FusionWebFetchError('invalid_url', 'fusion_web_fetch requires a valid absolute URL', error instanceof Error ? { cause: error } : {});
    }
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
        throw new FusionWebFetchError('unsupported_scheme', 'fusion_web_fetch supports only absolute http: and https: URLs', { url: rawUrl });
    }
...
function normalizeRedirectUrl(baseUrl, location) {
    let next;
    try {
        next = new URL(location, baseUrl);
    }
    catch (error) {
        throw new FusionWebFetchError('redirect_blocked', 'fusion_web_fetch redirect target is not a valid URL', error instanceof Error ? { url: baseUrl.toString(), cause: error } : { url: baseUrl.toString() });
    }
    try {
...
async function fetchOne(url, options, deadlineMs, redirectHop) {
    const normalized = normalizeRequestUrl(url.toString());
    const vetted = await vetHost(normalized.hostname, options, redirectHop, normalized.url.toString(), deadlineMs);
    const remainingMs = deadlineMs - options.now();
    if (remainingMs <= 0) {
        throw new FusionWebFetchError('request_timeout', 'fusion_web_fetch request timeout elapsed', {
...
async function vetHost(hostname, options, redirectHop, url, deadlineMs) {
    ...
    const literalAddress = { address: hostname, family: literalFamily === 4 ? 4 : 6 };
    const resolvedAddresses = literalFamily === 0
    ...
    for (const address of resolvedAddresses) {
        ...
        if (!classification.public && !options.allowBlockedAddressesForTests) {
            throwAddressError(redirectHop, `fusion_web_fetch blocked non-public address for ${hostname}: ${address.address}`, url);
        }
    }
...
function throwAddressError(redirectHop, message, url) {
    throw new FusionWebFetchError(redirectHop ? 'redirect_blocked' : 'blocked_address', message, { url });
}
async function resolveWithLookup(hostname, lookup, url, remainingMs) {
    if (remainingMs <= 0) {
    ...
    try {
        return await Promise.race([
            ...
            new Promise((_resolve, reject) => {
                timer = setTimeout(() => reject(new FusionWebFetchError('request_timeout', 'fusion_web_fetch request timeout elapsed during DNS lookup', { url })), Math.max(1, remainingMs));
            }),
        ]);
    }
    catch (error) {
        if (error instanceof FusionWebFetchError)
            throw error;
        throw new FusionWebFetchError('dns_failure', `DNS lookup failed for ${hostname}: ${error instanceof Error ? error.message : 'unknown error'}`, error instanceof Error ? { url, cause: error } : { url });
    }
    finally {
        if (timer !== undefined)
            clearTimeout(timer);
...
function executeRequest(url, selectedAddress, options, remainingMs) {
    return new Promise((resolve, reject) => {
        ...
        const request = options.request(url.protocol, buildRequestOptions(url, selectedAddress, options));
        timer = setTimeout(() => {
        ...
        request.once('response', (response) => {
            ...
            if (!addressMatches(selectedAddress.address, remoteAddress)) {
                response.destroy();
                settleReject(new FusionWebFetchError('blocked_address', `fusion_web_fetch socket remote address mismatch: expected ${selectedAddress.address}, got ${remoteAddress ?? 'missing'}`, errorDetails(url.toString(), response.statusCode)));
                return;
            }
            const remoteClass = classifyAddress(remoteAddress ?? '');
            if (!remoteClass.public && !options.allowBlockedAddressesForTests) {
                response.destroy();
                settleReject(new FusionWebFetchError('blocked_address', 'fusion_web_fetch socket remote address is blocked', errorDetails(url.toString(), response.statusCode)));
                return;
            }
            ...
            if (isRedirectStatus(status)) {
                ...
                if (location === undefined || location.trim().length === 0) {
                    settleReject(new FusionWebFetchError('http_error', `fusion_web_fetch redirect status ${String(status)} lacks Location`, {
                        url: url.toString(),
                        status,
                    }));
                    return;
            ...
            if (status < 200 || status > 299) {
                response.destroy();
                settleReject(new FusionWebFetchError('http_error', `fusion_web_fetch HTTP status ${String(status)}`, {
                    url: url.toString(),
                    status,
                }));
                return;
            ...
            try {
                ...
                response.destroy();
                settleReject(error instanceof FusionWebFetchError
                    ? error
            ...
            if (contentLength !== undefined && contentLength > options.maxResponseBytes) {
                response.destroy();
                settleReject(new FusionWebFetchError('response_too_large', 'fusion_web_fetch Content-Length exceeds response cap', {
                    url: url.toString(),
                    status,
                }));
                return;
            ...
            response.on('data', (chunk) => {
                ...
                if (nextBytesRead > options.maxResponseBytes) {
                    response.destroy();
                    settleReject(new FusionWebFetchError('response_too_large', 'fusion_web_fetch streamed body exceeds response cap', {
                        url: url.toString(),
                        status,
                    }));
                    return;
...
function buildRequestOptions(url, selectedAddress, options) {
    ...
    if (url.protocol === 'https:')
        requestOptions.servername = normalizeHostname(url.hostname);
    if (options.createConnection !== undefined)
        requestOptions.createConnection = options.createConnection;
    return requestOptions;
}
function forceSelectedLookup(selectedAddress) {
...
function defaultTransportRequest(protocol, options) {
...
function mapNetworkError(error, url, status) {
    if (error instanceof FusionWebFetchError)
        return error;
    return new FusionWebFetchError('network_error', `fusion_web_fetch network error: ${error.message}`, errorDetails(url, status, error));
}
function errorDetails(url, status, cause) {
    const details = { url };
    if (status !== undefined)
...
function unsupportedContentTypeError(contentType, url, status) {
    ...
    return new FusionWebFetchError('unsupported_content_type', `fusion_web_fetch unsupported content type: ${contentType.length === 0 ? 'missing' : contentType}`, { url, status });
}
function readContentLength(value, url, status) {
    if (value === undefined)
        return undefined;
...
async function extractContent(body, contentType, requestedFormat) {
    try {
        const mediaType = mediaTypeFromContentType(contentType);
        const decoded = decodeBody(body, contentType);
        if (mediaType === 'text/plain')
            return { content: decoded, format: 'text' };
        if (mediaType === 'text/markdown')
            return { content: decoded, format: 'markdown' };
        if (requestedFormat === 'text')
            return { content: htmlToText(decoded), format: 'text' };
        return { content: await htmlToMarkdown(decoded), format: 'markdown' };
    }
    catch (error) {
        if (error instanceof FusionWebFetchError)
            throw error;
        throw new FusionWebFetchError('extraction_failed', `fusion_web_fetch extraction failed: ${error instanceof Error ? error.message : 'unknown error'}`, error instanceof Error ? { cause: error } : {});
    }
}
function decodeBody(body, contentType) {
    ...
    try {
        ...
        throw new FusionWebFetchError('extraction_failed', `fusion_web_fetch response body could not be decoded as ${charset}`, error instanceof Error ? { cause: error } : {});
    }
}
...
function charsetFromContentType(contentType) {
    for (const parameter of contentType.split(';').slice(1)) {
        ...
        if (value.length === 0)
            throw new FusionWebFetchError('extraction_failed', 'fusion_web_fetch charset is empty');
        return value;
    }
    return undefined;
}
function stripAsciiQuotes(value) {
...
async function loadTurndownService() {
...
function normalizeTurndownLoadError(error) {
    if (isMissingTurndownDependency(error)) {
        const details = error instanceof Error ? { cause: error } : {};
        return new FusionWebFetchError('extraction_failed', 'fusion_web_fetch markdown extraction dependency "turndown" is missing from pi-background-tasks; repair the package install with `pi update --extensions` or `npm install --omit=dev --prefix <pi-background-tasks>`.', details);
    }
    if (error instanceof Error)
        return error;
    return new Error(`fusion_web_fetch markdown extraction dependency failed to load: ${String(error)}`);
}
function isMissingTurndownDependency(error) {
...
function stripUnsafeHtmlBlocks(html) {
    return html.replace(/<(script|style|noscript)\b[^>]*>[\s\S]*?<\/\1>/giu, '');
}
function replaceTablesWithTokens(html) {
    const replacements = [];
...
function tableToMarkdown(tableHtml) {
    ...
    for (const rowMatch of tableHtml.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/giu)) {
        ...
        const cells = [...rowHtml.matchAll(/<(?:th|td)\b[^>]*>([\s\S]*?)<\/(?:th|td)>/giu)].map((cellMatch) => inlineHtmlToText(cellMatch[1] ?? ''));
        if (cells.length > 0)
            rows.push(cells);
    }
    if (rows.length === 0)
...
function decodeHtmlEntities(value) {
    return value.replace(/&(#x[0-9a-f]+|#\d+|amp|lt|gt|quot|apos|nbsp);/giu, (_match, entity) => {
...
function codePointToString(value) {
...
function classifyAddress(address) {
    ...
    if (ipv4 !== undefined)
        return { public: !IPV4_DENY_RANGES.some(([base, prefix]) => ipv4InRange(ipv4, base, prefix)) };
    const ipv6 = parseIpv6Address(address);
    if (ipv6 !== undefined)
        return { public: !IPV6_DENY_RANGES.some(([base, prefix]) => ipv6InRange(ipv6, base, prefix)) };
    return { public: false };
}
function addressMatches(expected, actual) {
    ...
    return (expectedComparable !== undefined &&
...
function comparableAddress(address) {
    const ipv4 = parseIpv4Address(address);
    if (ipv4 !== undefined)
        return { family: 4, value: BigInt(ipv4) };
    const ipv6 = parseIpv6Address(address);
    if (ipv6 === undefined)
        return undefined;
    const mapped = ipv4FromMappedIpv6(ipv6);
    if (mapped !== undefined)
        return { family: 4, value: BigInt(mapped) };
...
function ipv4ToNumberLiteral(address) {
    const parsed = parseIpv4Address(address);
    if (parsed === undefined)
        throw new Error(`Invalid IPv4 literal in source: ${address}`);
    return parsed;
}
...
function ipv6ToBigIntLiteral(address) {
    const parsed = parseIpv6Address(address);
    if (parsed === undefined)
        throw new Error(`Invalid IPv6 literal in source: ${address}`);
    return parsed;
}
function ipv4FromMappedIpv6(address) {
    const mappedPrefix = ipv6ToBigIntLiteral('::ffff:0:0');
    if (!ipv6InRange(address, mappedPrefix, 96))
        return undefined;
