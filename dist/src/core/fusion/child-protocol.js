...
export const FUSION_CHILD_RESULT_SCHEMA_VERSION = 'pi-background-tasks.fusion-child-result.v4';
export const FUSION_CHILD_RESULT_PREFIX = '\u001ePI_FUSION_CHILD_RESULT ';
export const FUSION_CHILD_SETTLEMENT_SCHEMA_VERSION = 'pi-background-tasks.fusion-child-settlement.v3';
...
export const FUSION_TOOL_CALL_SEAL_SCHEMA_VERSION = 'pi-background-tasks.fusion-tool-call-seal.v1';
export const FUSION_TOOL_CALL_SEAL_SUFFIX = '.seal.json';
export const FUSION_RUNTIME_GUARD_SCHEMA_VERSION = 'pi-background-tasks.fusion-runtime-guard.v2';
...
export function serializeFusionChildResultRecords(records) {
    return Buffer.from(records.length === 0 ? '' : `${records.map((record) => JSON.stringify(record)).join('\n')}\n`, 'utf8');
}
function hasZeroUsage(record) {
...
export function isRecoverableFusionChildErrorRecord(record) {
    return (record.stop_reason === 'error' &&
        record.text_blocks.length === 0 &&
        record.text_sha256 === protocolSha256(Buffer.alloc(0)) &&
        hasZeroUsage(record) &&
        record.output_contract.recovery_role === 'none');
}
function isOversizedOriginal(record) {
    const output = record.output_contract;
    return (output.recovery_role === 'oversized_original' &&
...
function outputRecoveryProtocolInvalid(records) {
    const originals = records.flatMap((record, ordinal) => record.output_contract.recovery_role === 'oversized_original' ? [ordinal] : []);
    const replacements = records.flatMap((record, ordinal) => record.output_contract.recovery_role === 'replacement' ? [ordinal] : []);
    for (const record of records) {
        const output = record.output_contract;
        if (output.candidate_limit_bytes !== null &&
            output.candidate_limit_bytes !== FUSION_CANDIDATE_MAX_OUTPUT_BYTES) {
            return true;
        }
        if (output.recovery_role !== 'none' && output.candidate_limit_bytes === null)
            return true;
        if (output.recovery_role === 'oversized_original' && !isOversizedOriginal(record))
            return true;
    }
    if (originals.length === 0 && replacements.length === 0)
        return false;
    if (originals.length !== 1 || replacements.length !== 1)
        return true;
...
function finalCandidateOutputExceedsContract(records) {
    const final = records.at(-1);
    if (final === undefined)
        return false;
    const output = final.output_contract;
    return (output.candidate_limit_bytes === FUSION_CANDIDATE_MAX_OUTPUT_BYTES &&
        output.json_rendered_bytes > FUSION_CANDIDATE_MAX_OUTPUT_BYTES);
}
export function buildFusionChildSettlement(records, runtimeGuardFailed = false, cacheObservationFailed = false, outputRecoveryFailed = false) {
    const finalRecordIndex = records.length === 0 ? null : records.length - 1;
    const final = records.at(-1);
    const recoveredErrorOrdinals = records.flatMap((record, ordinal) => ordinal < records.length - 1 && isRecoverableFusionChildErrorRecord(record) ? [ordinal] : []);
    const recoveredOutputCapOrdinals = records.flatMap((record, ordinal) => ordinal < records.length - 1 && isOversizedOriginal(record) ? [ordinal] : []);
    const invalidRecovery = outputRecoveryProtocolInvalid(records);
    const invalidNonFinal = records.some((record, ordinal) => ordinal < records.length - 1 &&
        record.stop_reason !== 'toolUse' &&
        !isRecoverableFusionChildErrorRecord(record) &&
        !isOversizedOriginal(record));
    let failureReason = null;
    if (runtimeGuardFailed)
        failureReason = 'runtime_guard';
    else if (cacheObservationFailed)
        failureReason = 'cache_observation';
    else if (final === undefined)
        failureReason = 'no_records';
    else if (final.stop_reason !== 'stop')
        failureReason = 'final_not_stop';
    else if (outputRecoveryFailed ||
        invalidRecovery ||
        finalCandidateOutputExceedsContract(records)) {
        failureReason = 'output_recovery';
    }
    else if (invalidNonFinal)
        failureReason = 'invalid_non_final';
    return {
...
export function buildFusionChildResultMetadata(message, cacheObservation, outputContract = { candidateLimitBytes: null, recoveryRole: 'none' }) {
    if (outputContract.candidateLimitBytes !== null &&
    ...
    const textBlocks = message.content.flatMap((part) => part.type === 'text' && typeof part.text === 'string' ? [part.text] : []);
...
//# sourceMappingURL=child-protocol.js.map