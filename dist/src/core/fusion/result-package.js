...
import { FUSION_FAILURE_SUMMARY_ATTEMPT_CAP, FUSION_FAILURE_SUMMARY_EVIDENCE_CAP, FUSION_FAILURE_SUMMARY_INLINE_MESSAGE_BYTES, FUSION_FAILURE_SUMMARY_MAX_BYTES, assertFusionArtifactBasename, buildFusionRunProgress, } from './artifacts.js';
import { FUSION_COMMITTED_RESULT_SCHEMA_VERSION, FUSION_FAILURE_SUMMARY_SCHEMA_VERSION, FUSION_LEGACY_MANIFEST_SCHEMA_VERSION, FUSION_MANIFEST_SCHEMA_VERSION, FUSION_RESULT_SCHEMA_VERSION, FusionError, } from './types.js';
const SHA256_PATTERN = /^sha256:[0-9a-f]{64}$/u;
function isRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
function fail(message, artifactDir) {
...
function assertOnlyKeys(value, allowed, label, artifactDir) {
    const unexpected = Object.keys(value).filter((key) => !allowed.includes(key));
    if (unexpected.length > 0)
        fail(`${label} contains unexpected keys: ${unexpected.join(', ')}`, artifactDir);
}
function artifactRef(value, label, artifactDir) {
    if (!isRecord(value))
        fail(`${label} must be an object`, artifactDir);
...
function usage(value, artifactDir) {
    if (!isRecord(value))
        fail('details.usage must be an object', artifactDir);
    assertOnlyKeys(value, [
        ...
        'totalTokens',
        'cost',
    ], 'details.usage', artifactDir);
    const cost = value['cost'];
    if (!isRecord(cost))
        fail('details.usage.cost must be an object', artifactDir);
    assertOnlyKeys(cost, ['input', 'output', 'cacheRead', 'cacheWrite', 'total'], 'details.usage.cost', artifactDir);
    const finiteNonnegative = (entry, label) => {
    ...
    const cacheWrite1h = value['cacheWrite1h'] === undefined
        ? undefined
...
function resultDetails(value, expected) {
    if (!isRecord(value))
        fail('details must be an object', expected.artifactDir);
    assertOnlyKeys(value, [
        'schema_version',
        'run_id',
        'workflow',
    ...
    if ((context['kind'] !== 'session_projection' && context['kind'] !== 'clean_task') ||
        typeof context['policy_id'] !== 'string') {
        fail('details.context is invalid', expected.artifactDir);
    }
    assertOnlyKeys(toolPolicy, ['candidate_tools', 'evaluation_tools', 'merge_tools'], 'details.tool_policy', expected.artifactDir);
    const stringArray = (entry) => Array.isArray(entry) && entry.every((item) => typeof item === 'string');
    if (!stringArray(toolPolicy['candidate_tools']) ||
    ...
    assertOnlyKeys(models, ['candidates', 'evaluator', 'merger', 'thinking_level'], 'details.models', expected.artifactDir);
    if (!stringArray(models['candidates']) ||
    ...
    ], 'details.budget', expected.artifactDir);
    if (typeof budget['policy_id'] !== 'string' ||
    ...
    if (!Number.isSafeInteger(value['evaluator_attempts']) ||
    ...
    return {
        ...
        budget: {
            ...
            route_table: budget['route_table'],
            rate_sources: budget['rate_sources'],
            unknown_provider_warnings: [...budget['unknown_provider_warnings']],
            calibration_warnings: budget['calibration_warnings'],
        },
    };
}
...
async function readUtf8(path, label, artifactDir) {
    let bytes;
    try {
        bytes = await readFile(path);
    }
    catch (error) {
        fail(`${label} is unreadable: ${error instanceof Error ? error.message : String(error)}`, artifactDir);
    }
    let text;
    try {
...
async function readFailureUtf8(path, label, artifactDir, maxBytes) {
    try {
        ...
        if (metadata.size > maxBytes)
            fail(`${label} exceeds its bounded artifact size`, artifactDir);
    }
    catch (error) {
...
export async function readFusionCommittedResult(options) {
    const manifestFile = await readUtf8(join(options.artifactDirAbs, 'manifest.json'), 'manifest.json', options.artifactDir);
    let manifestValue;
    try {
        manifestValue = parseJsonText(manifestFile.text);
    }
    catch (error) {
        fail(`manifest.json is invalid JSON: ${error instanceof Error ? error.message : String(error)}`, options.artifactDir);
    }
    if (!isRecord(manifestValue))
        fail('manifest.json must be an object', options.artifactDir);
    ...
    if (!isRecord(artifacts))
        fail('manifest artifacts map is invalid', options.artifactDir);
    const manifestMerged = artifactRef(artifacts['merged.md'], 'manifest artifacts merged.md', options.artifactDir);
    const manifestResult = artifactRef(artifacts['result.json'], 'manifest artifacts result.json', options.artifactDir);
    ...
    const resultFile = await readUtf8(join(options.artifactDirAbs, 'result.json'), 'result.json', options.artifactDir);
    if (resultFile.bytes.length !== manifestResult.byte_length ||
    ...
    try {
        ...
        fail(`result.json is invalid JSON: ${error instanceof Error ? error.message : String(error)}`, options.artifactDir);
    }
    if (!isRecord(resultValue))
        fail('result.json must be an object', options.artifactDir);
    assertOnlyKeys(resultValue, ['schema_version', 'run_id', 'merged', 'details'], 'result.json', options.artifactDir);
    if (resultValue['schema_version'] !== FUSION_COMMITTED_RESULT_SCHEMA_VERSION ||
        resultValue['run_id'] !== options.runId) {
        fail('result.json identity mismatch', options.artifactDir);
    }
    const committedMerged = artifactRef(resultValue['merged'], 'result.json merged', options.artifactDir);
    ...
    const mergedFile = await readUtf8(join(options.artifactDirAbs, 'merged.md'), 'merged.md', options.artifactDir);
    if (mergedFile.bytes.length !== committedMerged.byte_length ||
...
function failureUnavailable(state, status) {
    return {
...
function failureString(value, label) {
    if (typeof value !== 'string')
        throw new Error(`${label} must be a string`);
    return value;
}
function failureInteger(value, label) {
    if (!Number.isSafeInteger(value) || Number(value) < 0)
        throw new Error(`${label} must be a nonnegative integer`);
    return Number(value);
}
function compareFailureText(left, right) {
    return left < right ? -1 : left > right ? 1 : 0;
}
function failureStage(value, label) {
    if (value === 'candidate' || value === 'evaluation' || value === 'merge')
        return value;
    throw new Error(`${label} is invalid`);
}
function failureUsage(value, artifactDir) {
    return usage(value, artifactDir);
}
function failureRef(value, label, artifactDir) {
    return artifactRef(value, label, artifactDir);
}
function sameFailureRef(left, right) {
    return (left.path === right.path &&
        left.byte_length === right.byte_length &&
        left.sha256 === right.sha256);
}
function trustedFailureManifest(value, options) {
    if (!isRecord(value))
        throw new Error('manifest must be an object');
    const schemaVersion = value['schema_version'];
    if (schemaVersion !== FUSION_MANIFEST_SCHEMA_VERSION &&
    ...
    for (const [name, ref] of Object.entries(artifactsValue)) {
        ...
        if (checked.path !== name)
            throw new Error('manifest artifact key/ref divergence');
        artifacts[name] = checked;
    }
    const attemptsValue = value['attempts'];
    if (!Array.isArray(attemptsValue))
        throw new Error('manifest attempts are invalid');
    const attempts = [];
    const classifications = {};
    const attemptArtifact = (entry, label) => {
        if (entry === undefined)
            return undefined;
        const name = failureString(entry, label);
        assertFusionArtifactBasename(name);
        if (artifacts[name] === undefined)
            throw new Error(`${label} is not manifest-bound`);
        return name;
    };
    for (const attemptValue of attemptsValue) {
        ...
        const partial = attemptArtifact(attemptValue['partial_response_path'], 'manifest partial_response_path');
        if (partial !== undefined)
            classifications[partial] = 'partial_stage_output';
        const recovery = attemptValue['output_recovery'];
        if (recovery !== undefined) {
            if (!isRecord(recovery))
                throw new Error('manifest output_recovery is invalid');
            const original = attemptArtifact(recovery['original_response_path'], 'manifest output_recovery.original_response_path');
            if (original === undefined)
                throw new Error('manifest output recovery has no original response');
            classifications[original] = 'oversized_original';
        }
    }
    attempts.sort((left, right) => compareFailureText(left.stage, right.stage) ||
...
function failureMessage(value) {
    if (!isRecord(value))
        throw new Error('failure message is invalid');
    ...
    if (!SHA256_PATTERN.test(sha256))
        throw new Error('failure message sha256 is invalid');
...
function failureList(value, label, cap, parseEntry) {
    if (!isRecord(value))
        throw new Error(`${label} is invalid`);
...
function failureAttempt(value) {
    if (!isRecord(value))
        throw new Error('failure attempt is invalid');
    ...
    if (attempt === 0)
        throw new Error('failure attempt number must be positive');
    return {
        stage,
        ...(slot === undefined ? {} : { slot }),
        attempt,
...
function failureEvidence(value, manifest, artifactDir) {
    if (!isRecord(value))
        throw new Error('failure evidence row is invalid');
...
function failureProgress(value, manifest, artifactDir) {
    if (!isRecord(value))
        throw new Error('failure progress is invalid');
    ...
    const stage = (entry, label, candidates) => {
        if (!isRecord(entry))
            throw new Error(`${label} is invalid`);
...
function parseFailureSummary(value, manifest, options) {
    if (!isRecord(value))
        throw new Error('failure summary must be an object');
    assertOnlyKeys(value, [
        'schema_version',
        'run_id',
        'workflow',
        ...
        'evidence_artifacts',
        'remediation_ids',
    ], 'failure summary', options.artifactDir);
    if (value['schema_version'] !== FUSION_FAILURE_SUMMARY_SCHEMA_VERSION ||
    ...
    if (!isRecord(failure))
        throw new Error('failure summary failure metadata is invalid');
    ...
    if (failure['attempt'] !== undefined)
        failureInteger(failure['attempt'], 'failure attempt');
    ...
    const attempts = failureList(value['attempts'], 'failure attempts', FUSION_FAILURE_SUMMARY_ATTEMPT_CAP, failureAttempt);
    const expectedAttempts = manifest.attempts;
    const expectedAttemptListedCount = Math.min(expectedAttempts.length, FUSION_FAILURE_SUMMARY_ATTEMPT_CAP);
    if (attempts.listed.length !== expectedAttemptListedCount ||
        attempts.omitted_count !== expectedAttempts.length - expectedAttemptListedCount ||
        canonicalJson(attempts.listed) !==
            canonicalJson(expectedAttempts.filter((_attempt, index) => index < expectedAttemptListedCount))) {
        throw new Error('failure attempt metadata diverges from manifest');
    }
    const evidence = failureList(value['evidence_artifacts'], 'failure evidence artifacts', FUSION_FAILURE_SUMMARY_EVIDENCE_CAP, (entry) => failureEvidence(entry, manifest, options.artifactDir));
    ...
    const expectedEvidenceListedCount = Math.min(expectedEvidence.length, FUSION_FAILURE_SUMMARY_EVIDENCE_CAP);
    if (evidence.listed.length !== expectedEvidenceListedCount ||
        evidence.omitted_count !== expectedEvidence.length - expectedEvidenceListedCount ||
        canonicalJson(evidence.listed) !==
            canonicalJson(expectedEvidence.filter((_evidence, index) => index < expectedEvidenceListedCount))) {
    ...
    if (!Array.isArray(remediationIds) ||
    ...
    if (manifest.error === undefined)
        throw new Error('manifest terminal error is unavailable');
...
function boundedFailureView(source, status, summaryRef) {
    const attempts = {
    ...
    const fits = () => Buffer.byteLength(canonicalJson(view), 'utf8') <= FAILURE_VIEW_MAX_BYTES;
    ...
    if (!fits())
        throw new Error('failure result view exceeds its byte budget without a safe whole-section omission');
    return view;
}
function legacyFailureSource(manifest) {
    const message = manifest.error === undefined
        ? undefined
        : (() => {
...
export async function readFusionFailureResult(options) {
    let manifest;
    try {
    ...
    if (manifest.schemaVersion !== FUSION_MANIFEST_SCHEMA_VERSION ||
    ...
    try {
        ...
        const file = await readFailureUtf8(join(options.artifactDirAbs, summaryRef.path), 'failure-summary.json', options.artifactDir, FUSION_FAILURE_SUMMARY_MAX_BYTES);
        if (file.bytes.length !== summaryRef.byte_length ||
...
//# sourceMappingURL=result-package.js.map