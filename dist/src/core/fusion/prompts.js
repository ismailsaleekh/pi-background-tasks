...
import { FUSION_EVALUATION_SCHEMA_VERSION, FUSION_VALIDATE_CAPABILITY, FUSION_VALIDATE_CANDIDATE_SCHEMA_VERSION, FusionError, } from './types.js';
/**
...
Do not invent process metadata. Do not mention provider names, model names, slots, or hidden workflow details. Do not specialize the answer; each child receives the same instruction. Output only the answer text.`;
export function fusionCandidateSystemPrompt(capability) {
    switch (capability) {
        case 'reason':
 ...
 */
export function fusionValidateCandidateSystemPrompt(capability) {
    if (capability !== FUSION_VALIDATE_CAPABILITY) {
        throw new FusionError(`fusion validate candidates always run with the ${FUSION_VALIDATE_CAPABILITY} capability; received ${String(capability)}`, { code: 'orchestration_failed', childCreated: false });
    }
...
export function buildBlindEvaluationInput(canonicalInput, candidates, validationSourceFindings) {
    const input = {
...
export function buildEvaluationRepairPrompt(input) {
    return canonicalJson(input);
}
export function buildMergeInput(canonicalInput, candidates, evaluation) {
    return {
