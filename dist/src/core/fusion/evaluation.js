...
import { FUSION_CANDIDATE_IDS, FUSION_EVALUATION_SCHEMA_VERSION, FUSION_VALIDATE_CANDIDATE_SCHEMA_VERSION, FusionError, } from './types.js';
...
function closed(record, keys, label, errors) {
    const expected = new Set(keys);
    for (const key of Object.keys(record)) {
        if (!expected.has(key))
            errors.push(`${label} contains unknown key ${key}`);
    }
...
function nonBlankString(value, label, errors) {
...
function candidateId(value, label, errors) {
    if (value === 'A' || value === 'B' || value === 'C')
        return value;
    errors.push(`${label} must be A, B, or C`);
    return undefined;
}
function tuple3(items, label, errors) {
    if (items.length !== 3) {
...
function parseAssessment(value, label, errors) {
    if (!isRecord(value)) {
        errors.push(`${label} must be an object`);
        return undefined;
    }
    closed(value, ['candidate_id', 'summary', 'strengths', 'limitations', 'useful_contributions', 'risks'], label, errors);
...
function parsePosition(value, label, errors) {
    if (!isRecord(value)) {
        errors.push(`${label} must be an object`);
        return undefined;
    }
...
function parseConflict(value, label, errors) {
    if (!isRecord(value)) {
        errors.push(`${label} must be an object`);
        return undefined;
    }
...
function parseContribution(value, label, errors) {
    if (!isRecord(value)) {
        errors.push(`${label} must be an object`);
        return undefined;
    }
    ...
    if (id === undefined || contribution === undefined)
        return undefined;
    return { candidate_id: id, contribution };
}
function parseContributionList(value, label, errors) {
    if (!Array.isArray(value)) {
        errors.push(`${label} must be an array`);
        return undefined;
    }
...
function parseSynthesisPlan(value, label, errors) {
    if (!isRecord(value)) {
        errors.push(`${label} must be an object`);
        return undefined;
    }
    ...
    if (include === undefined || resolve === undefined || avoid === undefined)
        return undefined;
    return { must_include: include, must_resolve: resolve, must_avoid: avoid };
}
function parseAssessmentList(value, label, errors) {
    if (!Array.isArray(value)) {
        errors.push(`${label} must be an array`);
        return undefined;
    }
    ...
    for (const id of FUSION_CANDIDATE_IDS) {
        if (!ids.has(id))
            errors.push(`${label} must contain candidate ${id}`);
    }
    if (ids.size !== parsed.length)
        errors.push(`${label} candidate_id values must be unique`);
    return tuple3(parsed, label, errors);
}
function parseConflictList(value, label, errors) {
    if (!Array.isArray(value)) {
        errors.push(`${label} must be an array`);
        return undefined;
    }
...
function parseValidationGroups(value, label, errors) {
    if (!Array.isArray(value)) {
        errors.push(`${label} must be an array`);
        return [];
    }
    ...
    for (const [index, item] of value.entries()) {
        ...
        closed(item, ['group_id', 'source_ids', 'severity', 'location', 'evidence', 'impact', 'summary', 'rationale'], itemLabel, errors);
        ...
        const severity = nonBlankString(item['severity'], `${itemLabel}.severity`, errors);
...
function parseValidationAccounting(value, label, errors) {
    if (!isRecord(value)) {
        errors.push(`${label} must be an object`);
        return undefined;
    }
        ...
        for (const [index, item] of decisionsRaw.entries()) {
            ...
            for (const key of Object.keys(item)) {
                if (!allowedDecisionKeys.has(key))
                    errors.push(`${itemLabel} contains unknown key ${key}`);
            }
            for (const key of ['source_id', 'disposition', 'rationale']) {
                if (!Object.prototype.hasOwnProperty.call(item, key))
                    errors.push(`${itemLabel} is missing key ${key}`);
            }
            ...
            if (disposition !== undefined && disposition !== 'include' && disposition !== 'exclude')
                errors.push(`${itemLabel}.disposition invalid`);
            if (sourceId !== undefined && (disposition === 'include' || disposition === 'exclude') && rationale !== undefined) {
                const decision = { source_id: sourceId, disposition, rationale };
                if (group !== undefined)
                    decision.group_id = group;
                decisions.push(decision);
            }
        }
    }
    const accounting = { findings, decisions, groups };
    errors.push(...validateFusionFindingAccounting(accounting));
    return accounting;
}
export function validateFusionEvaluation(value) {
    const errors = [];
    if (!isRecord(value))
        return { ok: false, errors: ['evaluation must be a JSON object'] };
    ...
    for (const key of Object.keys(value)) {
        if (!evaluationAllowed.has(key))
            errors.push(`evaluation contains unknown key ${key}`);
    }
    for (const key of ['schema_version', 'candidate_assessments', 'agreements', 'conflicts', 'synthesis_plan']) {
        if (!Object.prototype.hasOwnProperty.call(value, key))
            errors.push(`evaluation is missing key ${key}`);
    }
    ...
    if (validationAccounting !== undefined)
        parsedValue.validation_accounting = validationAccounting;
    return { ok: true, value: parsedValue };
}
export function parseFusionEvaluation(text) {
    let parsed;
    try {
...
export function boundedEvaluationErrors(errors) {
    const bounded = [];
    let total = 0;
    for (const error of errors) {
        if (bounded.length >= MAX_REPAIR_ERROR_COUNT)
            break;
        const perError = error.length <= MAX_REPAIR_ERROR_CHARS
            ? error
...
export function formatEvaluationErrors(errors) {
    return boundedEvaluationErrors(errors).join('; ');
}
function parseValidationCandidateFinding(value, label, errors) {
    if (!isRecord(value)) {
        errors.push(`${label} must be an object`);
        return undefined;
    }
    closed(value, ['severity', 'location', 'evidence', 'impact', 'summary'], label, errors);
    const severity = nonBlankString(value['severity'], `${label}.severity`, errors);
    ...
    const summary = nonBlankString(value['summary'], `${label}.summary`, errors);
    if (severity !== undefined && !['critical', 'high', 'minor'].includes(severity))
        errors.push(`${label}.severity invalid`);
    if (severity === undefined || location === undefined || evidence === undefined || impact === undefined || summary === undefined)
        return undefined;
 ...
 */
function fencedValidationCandidateJson(text) {
    ...
    if (closing < 0 || trimmed.slice(closing + 3).includes('```'))
        return undefined;
    if (trimmed.slice(closing + 3).trim().length > 0)
        return undefined;
    const preamble = trimmed.slice(0, opening.index).trim();
    if (Buffer.byteLength(preamble, 'utf8') > 2_000 || preamble.includes('```'))
        return undefined;
 ...
 */
export function recoverFencedFusionValidationCandidateReport(text, candidateId) {
...
export function parseFusionValidationCandidateReport(text, candidateId) {
    let parsed;
    try {
        parsed = parseJsonText(text);
    }
    catch (error) {
        throw new FusionError(`validation candidate ${candidateId} output must be structured JSON only: ${errorText(error)}`, { code: 'evaluation_invalid', stage: 'candidate' });
    }
    const errors = [];
    if (!isRecord(parsed)) {
    ...
    if (parsed['schema_version'] !== FUSION_VALIDATE_CANDIDATE_SCHEMA_VERSION)
        errors.push('validation candidate report.schema_version mismatch');
    const rawFindings = parsed['findings'];
    const findings = [];
    if (!Array.isArray(rawFindings))
        errors.push('validation candidate report.findings must be an array');
    else {
...
export function stableFusionFindingId(candidateId, ordinal) {
...
export function validateFusionFindingAccounting(accounting) {
    const errors = [];
    const sourceIds = new Set();
    const perCandidateOrdinal = { A: 0, B: 0, C: 0 };
    for (const [index, finding] of accounting.findings.entries()) {
        ...
        if (!['critical', 'high', 'minor'].includes(finding.severity))
            errors.push(`${label}.severity invalid`);
        for (const key of ['location', 'evidence', 'impact', 'summary']) {
            if (finding[key].trim().length === 0)
                errors.push(`${label}.${key} must be non-blank`);
        }
        if (sourceIds.has(finding.id))
    ...
    for (const [index, decision] of accounting.decisions.entries()) {
        ...
        if (!sourceIds.has(decision.source_id))
            errors.push(`${label}.source_id does not name a candidate finding`);
        if (accounted.has(decision.source_id))
            errors.push(`${label}.source_id accounted more than once`);
        accounted.add(decision.source_id);
        if (decision.disposition !== 'include' && decision.disposition !== 'exclude')
            errors.push(`${label}.disposition invalid`);
        if (decision.rationale.trim().length === 0)
            errors.push(`${label}.rationale must be non-blank`);
    ...
    for (const id of sourceIds) {
        if (!accounted.has(id))
            errors.push(`source finding ${id} was not accounted exactly once`);
    }
    const groupsById = new Map();
    for (const [index, group] of accounting.groups.entries()) {
        const label = `group[${String(index)}]`;
        if (groupsById.has(group.group_id))
            errors.push(`${label}.group_id duplicate`);
        groupsById.set(group.group_id, group);
        if (!['critical', 'high', 'minor'].includes(group.severity))
            errors.push(`${label}.severity invalid`);
        for (const key of ['location', 'evidence', 'impact', 'summary', 'rationale']) {
            if (group[key].trim().length === 0)
                errors.push(`${label}.${key} must be non-blank`);
        }
        if (group.source_ids.length === 0)
            errors.push(`${label}.source_ids must not be empty`);
        const groupSourceIds = new Set();
        for (const sourceId of group.source_ids) {
            if (!sourceIds.has(sourceId))
                errors.push(`${label}.source_ids contains unknown finding ${sourceId}`);
            if (groupSourceIds.has(sourceId))
                errors.push(`${label}.source_ids contains duplicate ${sourceId}`);
            groupSourceIds.add(sourceId);
        }
    }
    const includedByGroup = new Map();
    for (const decision of accounting.decisions) {
        if (decision.disposition !== 'include' || decision.group_id === undefined)
            continue;
        ...
        const members = includedByGroup.get(decision.group_id) ?? new Set();
    ...
    for (const group of accounting.groups) {
        ...
        const actual = [...(includedByGroup.get(group.group_id) ?? new Set())].sort();
...
function sanitizeValidationRationale(value) {
    return value
        .replace(/\b[ABC]-F\d{3}\b/gu, 'source finding')
        .replace(/\bcandidate [ABC]\b/giu, 'one reviewer')
        .replace(/\b[ABC]:\s*/gu, '');
}
export function renderValidatedFusionValidationReport(accounting, coverage) {
    ...
    const renderedFindings = [...accounting.groups].sort((left, right) => severityOrder[left.severity] - severityOrder[right.severity] ||
    ...
    if (exclusions.length > 0) {
        ...
        for (const decision of exclusions)
            lines.push(`- ${sanitizeValidationRationale(decision.rationale)}`);
        lines.push('');
    }
    if (coverage !== undefined) {
        lines.push('## Verified', '');
        if (coverage.verified.length === 0)
            lines.push('- No verification statements were provided.');
        else
            for (const item of coverage.verified)
                lines.push(`- ${item}`);
        lines.push('', '## Limitations', '');
        if (coverage.limitations.length === 0)
            lines.push('- No limitations were provided.');
        else
            for (const item of coverage.limitations)
                lines.push(`- ${item}`);
        lines.push('');
    }
    return lines.join('\n').trimEnd();
}
export function assertMergerFindingCoverage(accounting, renderedGroupIds) {
    const errors = [...validateFusionFindingAccounting(accounting)];
    const included = new Set(accounting.groups.map((group) => group.group_id));
    const rendered = new Set(renderedGroupIds);
    for (const id of included)
        if (!rendered.has(id))
            errors.push(`merger dropped included group ${id}`);
    for (const id of rendered)
        if (!included.has(id))
            errors.push(`merger invented or revived group ${id}`);
    if (errors.length > 0) {
