...
import { DELEGATE_RECEIPT_SCHEMA_VERSION, DELEGATE_CAPABILITIES, } from './core/delegate/types.js';
import { verifyDelegateSeedBytes } from './core/delegate/seed.js';
import { DELEGATE_FINALIZATION_INPUT_RESERVE_TOKENS, DELEGATE_FINALIZATION_TRIGGER_TOKENS, evaluateDelegateRuntimeBudget, } from './core/delegate/budget.js';
import { utf8ByteClassBreakdown } from './core/context/token-budget.js';
import { assertWellFormedUtf8, buildDelegateResultPackage, serializeDelegateResultPackage, } from './core/delegate/result-package.js';
/**
 ...
 */
const ACCEPTED_STOP_REASONS = new Set(['stop']);
 ...
 */
function suppressedMessages(messages) {
...
function encodeToolResultContent(content) {
    if (content.length === 1) {
...
function addUsage(current, delta) {
    const prior = current ?? {
        input: 0,
        output: 0,
        cacheRead: 0,
...
function commitFileSync(absPath, data) {
...
function emptyContentMeasurement() {
    return { bytes: 0, multibyteBytes: 0, denseBytes: 0 };
}
function addContentMeasurement(target, delta) {
...
function messageContentMeasurement(content) {
    if (typeof content === 'string')
        return stringMeasurement(content);
    ...
    for (const part of content) {
        ...
        if (typeof text === 'string')
            addContentMeasurement(total, stringMeasurement(text));
        const thinkingText = Reflect.get(part, 'thinking');
        if (typeof thinkingText === 'string')
            addContentMeasurement(total, stringMeasurement(thinkingText));
        const args = Reflect.get(part, 'arguments');
        if (args !== undefined)
            addContentMeasurement(total, stringMeasurement(JSON.stringify(args) ?? ''));
        const data = Reflect.get(part, 'data');
        if (typeof data === 'string')
            addContentMeasurement(total, stringMeasurement(data));
    }
    return total;
}
/** Complete retained input measured the same way the admission plan measured the seed. */
function retainedInputMeasurement(messages, systemPrompt) {
    const total = stringMeasurement(systemPrompt);
    for (const message of messages)
        addContentMeasurement(total, messageContentMeasurement(Reflect.get(message, 'content')));
    return total;
}
const ArtifactReadParams = Type.Object({
    artifact: Type.String({
        description: 'Relative artifact path exactly as named in a spill receipt, for example spill/t0001-c0000-abc.bin',
    }),
...
export default function delegateChildExtension(pi) {
    ...
    function remainingGrowthTokens() {
    ...
    function requestFinalization(reason) {
    ...
    function accountRetainedGrowth(bytes, source) {
        state.retainedGrowthTokens += bytes;
        if (state.retainedGrowthBudgetTokens !== undefined &&
            remainingGrowthTokens() <= DELEGATE_FINALIZATION_TRIGGER_TOKENS) {
            requestFinalization(`${source} left ${String(remainingGrowthTokens())} protected retained-growth tokens`);
        }
    }
    function writeRuntimeBudgetRecord() {
        if (state.runtimeBudgetWritten)
            return;
        const latest = state.contextMeasurements.at(-1);
        const firstEstimate = state.contextMeasurements[0]?.estimated_input_tokens;
        const calibrationViolation = firstEstimate !== undefined &&
            ...
            ? {
                ...
                tokens_under_forecast: state.firstRequestObservedInputTokens - firstEstimate,
            }
            : null;
        commitFileSync(join(artifactDirAbs, 'runtime-budget.json'), utf8(`${JSON.stringify({
            ...
            spilled_tool_result_bytes: state.spilled.reduce((sum, receipt) => sum + receipt.byte_length, 0),
     ...
     */
    function commitResult(stopReason) {
        if (state.committed)
            return;
        if (state.terminal !== undefined) {
    ...
    function writeTerminalRecord(terminal) {
        writeRuntimeBudgetRecord();
        commitFileSync(join(artifactDirAbs, 'child-terminal.json'), utf8(`${JSON.stringify({
    ...
    pi.registerTool({
        ...
        description: 'Read an exact byte range from a spilled tool-result artifact as lossless base64. Returns the complete requested range or fails; it never decodes arbitrary bytes as UTF-8, returns fewer bytes, or clamps the request.',
        ...
        prepareArguments(args) {
            ...
            const artifact = Reflect.get(args, 'artifact');
            const offset = Reflect.get(args, 'offset');
            const length = Reflect.get(args, 'length');
        ...
        execute(_toolCallId, params) {
            ...
            if (!pathInside(artifactDirAbs, absPath)) {
                throw new Error(`delegate_read_artifact path ${params.artifact} escapes the delegate artifact directory`);
            }
            const bytes = readFileSync(absPath);
            const end = params.offset + params.length;
            if (end > bytes.length) {
                throw new Error(`delegate_read_artifact requested bytes ${String(params.offset)}..${String(end)} but ${params.artifact} is ${String(bytes.length)} bytes; the read is refused rather than silently shortened`);
            }
            const slice = bytes.subarray(params.offset, end);
            if (slice.length !== params.length) {
                throw new Error(`delegate_read_artifact returned ${String(slice.length)} of ${String(params.length)} requested bytes`);
            }
            ...
            const availableInlineBytes = Math.max(0, Math.min(seed.limits.max_tool_result_bytes, remainingGrowthTokens() - DELEGATE_FINALIZATION_TRIGGER_TOKENS));
            if (responseBytes > availableInlineBytes) {
                if (availableInlineBytes === 0) {
                    ...
                    throw new Error(`delegate_read_artifact cannot retain another artifact range because protected final-answer runway is active; stop reading and answer from gathered evidence`);
                }
                throw new Error(`delegate_read_artifact requested ${String(params.length)} raw bytes whose lossless base64 response is ${String(responseBytes)} bytes, but current protected runway permits at most ${String(availableInlineBytes)} inline bytes; request a smaller exact range`);
            }
    ...
    pi.on('context', (event, ctx) => {
        ...
        try {
            ...
            }, seed.limits.allowed_input_tokens, seed.route);
            if (state.retainedGrowthBudgetTokens === undefined) {
                state.retainedGrowthBudgetTokens = Math.max(0, verdict.allowedTokens -
            ...
            if (verdict.measuredTokens >=
                verdict.allowedTokens - DELEGATE_FINALIZATION_INPUT_RESERVE_TOKENS) {
                requestFinalization(`advisory retained-input estimate reached ${String(verdict.measuredTokens)} of ${String(verdict.allowedTokens)} allowed tokens`);
            }
            if (!state.finalizationRequested)
                return undefined;
            ...
            latch('child_result_invalid', `delegate context measurement failed and the run was stopped rather than dispatched unguarded: ${error instanceof Error ? error.message : String(error)}`);
            try {
                ctx.abort();
            }
            catch {
    ...
    pi.on('tool_call', (_event, ctx) => {
        ...
        if (state.toolCalls > seed.limits.max_tool_calls) {
            latch('child_tool_call_limit', `delegate child exceeded its ${String(seed.limits.max_tool_calls)} tool-call limit`);
            ctx.abort();
            return {
                block: true,
    ...
    pi.on('tool_result', (event) => {
        ...
        try {
            ...
            latch('artifact_spill_failed', `delegate tool-result guard failed and the payload was withheld: ${error instanceof Error ? error.message : String(error)}`);
            return {
                content: [
                    {
                        type: 'text',
    ...
    function guardToolResult(event) {
        ...
        if (state.totalToolOutputBytes > seed.limits.max_total_tool_output_bytes) {
            latch('aggregate_tool_output_cap', `delegate child accumulated ${String(state.totalToolOutputBytes)} bytes of tool output, exceeding its ${String(seed.limits.max_total_tool_output_bytes)}-byte cap`);
        ...
        const contextPressure = state.retainedGrowthBudgetTokens === undefined || payload.length > remainingGrowthTokens();
        ...
        try {
            ...
            latch('artifact_spill_failed', `delegate could not spill a ${String(payload.length)}-byte tool result: ${error instanceof Error ? error.message : String(error)}`);
            return {
                content: [
                    {
                        type: 'text',
        ...
        if (contextPressure)
            state.contextPressureSpillBytes += payload.length;
    ...
    pi.on('turn_start', () => {
        ...
        if (state.turns > seed.limits.max_turns) {
            latch('child_turn_limit', `delegate child exceeded its ${String(seed.limits.max_turns)} turn limit`);
        }
    });
    pi.on('message_end', (event) => {
        if (event.message.role !== 'assistant')
            return;
        const provider = Reflect.get(event.message, 'provider');
        const model = Reflect.get(event.message, 'model');
        const stopReason = Reflect.get(event.message, 'stopReason');
        const attestation = {
        ...
        if (attestation.provider !== seed.route.provider ||
            attestation.model !== seed.route.model) {
            latch('route_mismatch', `delegate child produced an assistant message on ${attestation.provider}/${attestation.model}, but the pinned route is ${seed.route.qualified_id}`);
        }
        ...
        if (observedUsage === undefined) {
            ...
            if (!state.usageIncomplete)
                state.usageUnavailableReason = undefined;
        }
        const content = Reflect.get(event.message, 'content');
        if (!Array.isArray(content))
            return;
        ...
        for (const part of content) {
            ...
            if (state.answerBytes > seed.limits.max_answer_bytes) {
                latch('child_capture_limit', `delegate child answer text reached ${String(state.answerBytes)} bytes, exceeding its ${String(seed.limits.max_answer_bytes)}-byte capture contract; the transcript is preserved and no prefix is committed`);
                continue;
            }
...
//# sourceMappingURL=delegate-child-extension.js.map